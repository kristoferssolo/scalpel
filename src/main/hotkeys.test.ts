...
const overlayMockState: {
  overlayWindow: { isDestroyed: () => boolean; isFocused: () => boolean } | null
...
vi.mock('./overlay', () => ({
  getOverlayWindow: () => overlayMockState.overlayWindow,
...
async function loadHotkeys(onEscape: () => void) {
  ...
  overlayMockState.overlayWindow = null
...
describe('Escape globalShortcut sync', () => {
  ...
  it('reentrant unregister: onEscape synchronously hiding the overlay does not throw and leaves the shortcut cleanly re-registerable', async () => {
    // Mirrors the real chain: onEscape -> hideOverlay() -> overlay-visibility
    // listener -> syncEscapeShortcut(), all synchronously inside the
    // globalShortcut callback that is still running.
    const onEscape = vi.fn(() => {
      overlayMockState.visibilityListener?.(false)
    })
    await loadHotkeys(onEscape)
  ...
  it('uiohook-only path: shortcut never registers when the game is unfocused but the overlay window is focused', async () => {
    const onEscape = vi.fn()
    await loadHotkeys(onEscape)
    overlayControllerState.targetHasFocus = false
    overlayMockState.overlayWindow = { isDestroyed: () => false, isFocused: () => true }
