...
const PSEUDO_CONTRIBUTIONS: Record<string, PseudoContribution[]> = {}
...
export function matchItemMods(
  ...
  const pseudoAccumulator: Record<string, PseudoAccumulatorEntry> = {}
  // Hoisted ahead of the implicit loop so that pseudo accumulation can gate
  // weapon-only paths (e.g. consolidated added-ele-damage pseudo skips weapons).
  ...
  const pseudoFilters: StatFilter[] = Object.entries(pseudoAccumulator)
